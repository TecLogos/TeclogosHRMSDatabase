...
SELECT @SalesDeptID = ID
FROM [Department]
WHERE [Name] = 'Sales';
SELECT @FinanceDeptID = ID
FROM [Department]
WHERE [Name] = 'Finance';
SELECT @AdminDeptID = ID
FROM [Department]
WHERE [Name] = 'Administration';
SELECT @MarketingDeptID = ID
FROM [Department]
WHERE [Name] = 'Marketing';
SELECT @OpsDeptID = ID
FROM [Department]
WHERE [Name] = 'Operations';
SELECT @SoftEngDesignID = ID
FROM [Designation]
WHERE [Name] = 'Software Engineer';
SELECT @TeamLeadDesignID = ID
FROM [Designation]
WHERE [Name] = 'Team Lead';
SELECT @SeniorSoftEngDesignID = ID
FROM [Designation]
WHERE [Name] = 'Senior Software Engineer';
SELECT @ManagerDesignID = ID
FROM [Designation]
WHERE [Name] = 'General Manager';
SELECT @DirectorDesignID = ID
FROM [Designation]
WHERE [Name] = 'Director';
SELECT @GeneralShiftID = ID
FROM [Shifts]
WHERE [Name] = 'General Shift';
SELECT @MorningShiftID = ID
FROM [Shifts]
WHERE [Name] = 'Morning Shift';
SELECT @EveningShiftID = ID
FROM [Shifts]
WHERE [Name] = 'Evening Shift';
SELECT @EarnedLeaveID = ID
FROM [LeaveType]
WHERE [Name] = 'Earned leave';
SELECT @SickLeaveID = ID
FROM [LeaveType]
WHERE [Name] = 'Sick Causal Leaves';
SELECT @CompOffID = ID
FROM [LeaveType]
WHERE [Name] = 'Comp - Off';
SELECT @CasualLeaveID = ID
FROM [LeaveType]
...
FROM [Roles]
WHERE [Name] = 'Manager';
SELECT @HRMgrRoleID = ID
FROM [Roles]
WHERE [Name] = 'HR Manager';
SELECT @AdminRoleID = ID
FROM [Roles]
WHERE [Name] = 'Admin';
SELECT @FatherRelID = ID
FROM [RelationType]
WHERE [Name] = 'Father';
SELECT @MotherRelID = ID
FROM [RelationType]
WHERE [Name] = 'Mother';
SELECT @SpouseRelID = ID
FROM [RelationType]
WHERE [Name] = 'Spouse';
SELECT @AadhaarTypeID = ID
FROM [IdentifierType]
WHERE [Name] = 'Aadhaar Card';
SELECT @PANTypeID = ID
FROM [IdentifierType]
WHERE [Name] = 'PAN Card';
SELECT @DrivingLicenseTypeID = ID
FROM [IdentifierType]
WHERE [Name] = 'Driving License';
SELECT @PassportTypeID = ID
FROM [IdentifierType]
WHERE [Name] = 'Passport';
